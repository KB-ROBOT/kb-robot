...
public class UserController {
	...
	* @throws
	 ...
	 * @return
	 */
	@RequestMapping(params="changewexinaccount")
	@ResponseBody
	public AjaxJson changeWexinAccount(HttpServletRequest request){
		AjaxJson j = new AjaxJson();
		TSUser user = ResourceUtil.getSessionUserName();
		if(user==null){
			j.setSuccess(false);
		...
		attr.put("weixinAccountList", weixinAccountList);
		j.setSuccess(true);
		j.setAttributes(attr);
		return j;//"system/user/changeweixinaccount";
	}
	 ...
	 * @return
	 */
	@RequestMapping(params = "saveweixinaccount")
	@ResponseBody
	public AjaxJson saveWeixinAccount(HttpServletRequest request,HttpServletResponse response){
		AjaxJson j = new AjaxJson();
		j.setSuccess(Boolean.FALSE);
		TSUser user = ResourceUtil.getSessionUserName();
		if(user!=null){
			...
			if(StringUtils.isNotEmpty(accountId)){
				...
				List<WeixinAccountEntity> resultList = weixinAccountService.findByProperty(WeixinAccountEntity.class, "id", accountId);
				if(resultList!=null&&!resultList.isEmpty()){
					//重置session
					request.getSession().setAttribute(WeiXinConstants.WEIXIN_ACCOUNT, resultList.get(0));
