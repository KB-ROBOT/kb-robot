...
<script type="text/javascript">
	...
	function imageChange(liSeq) {
		$("#fileupload_"+liSeq).fileupload({
			...
			progressall : function(e, data) {
				...
				$("#progress_"+liSeq+" .progress-bar").css(
				    'width',
				    progress + '%'
				); 
			},
	...
	$(document).ready(function() {
		...
		$('#addline').click(function(e) {
			...
			m += '<label class="m10">图文内容</label><textarea style="height:200px;width:100%;" type="text/plain" name=\"content\" id="content_'+i+'"></textarea>';
			m += '</div>';
			m += '</div>';
			m += '</li>';
			$('.right_list').append(m);
